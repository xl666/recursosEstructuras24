...
class Arbol():
    ...
    def podar(self, indice):
        ...
        if nodo_a_borrar.padre:
            for j in range(len(nodo_a_borrar.padre.hijos)):
                if nodo_a_borrar.padre.hijos[j].indice==nodo_a_borrar.indice:
                    del(nodo_a_borrar.padre.hijos[j])
                    return
        else:
            self.raiz = None
        return 
