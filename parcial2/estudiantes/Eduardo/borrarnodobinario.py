...
class Arbol_binario():
    ...
    def __repr__(self) -> str:
        ...
        return self._imprimir_arbol(self.raiz, 0).strip()
    ...
    def _imprimir_arbol(self, nodo, nivel):
        if not nodo:
            return ""
        result = "| " * nivel + str(nodo) + "\n"
        if nodo.izquierda:
            result += self._imprimir_arbol(nodo.izquierda, nivel + 1)
        if nodo.derecha:
            result += self._imprimir_arbol(nodo.derecha, nivel + 1)
        return result
...
if __name__ == "__main__":
    ...
    for _ in range(N):
        indice, valor = input().strip().split(':')
        indice = int(indice)  
