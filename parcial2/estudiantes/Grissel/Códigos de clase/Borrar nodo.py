class Nodo():
    ...
    def __repr__(self):
        return str(self.indice) #convertir a cadena
...
class Arbol():
    ...
    def regresar_padre(self, indice: int) -> int:
        ...
        """
        nodo_interes = self.regresarNodo(indice)
        if not nodo_interes or not nodo_interes.padre: #si nodo no existe o si no tiene padre regresar Nada
            return None
    ...
    def borrar_nodo(self, indice): #eliminar nodo del arbol
        nodo = self.regresarNodo(indice) #encontrar nodo con índice
        if not nodo: #si no existe el nodo regresar Falso
            return False
        if nodo.padre: #si nodo tiene padre eliminar nodo
            nodo.padre.hijos.remove(nodo)
        else:
            self.raiz = None #si nodo no tiene padre, el nodo a eliminar es raíz
        return True
