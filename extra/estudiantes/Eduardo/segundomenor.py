...
def leer_arbol(n: int) -> Arbol_binario:
    """
    Regresa un árbol de acuerdo a como lo pasa el sistema
    """
    arbol = Arbol_binario()
    for _ in range(n):
        entrada = input().strip()
...
def encontrar_segundo_menor(arbol): 
    def inorder(nodo, result):
        if nodo is not None:
            inorder(nodo.izquierda, result)
            result.append(nodo.indice)
            inorder(nodo.derecha, result)

    indices = []
    inorder(arbol.raiz, indices)
    return indices[1]

if __name__ == '__main__':
    n = int(input())
    arbol = leer_arbol(n)
    segundo_menor = encontrar_segundo_menor(arbol)
    print(segundo_menor)